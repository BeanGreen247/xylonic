...
- `context/ImageCacheContext.tsx` — **not a context provider** (collapsed 2026-09-08). Module-level singleton store: `runInit()` initialises `imageCacheService` + `searchCacheService`, builds the coverArt alias map and prewarms the top 80 artist images; `ensureStarted()` runs it once + wires the `auth-changed`/`logout`/`storage` window listeners on first `useImageCache()` call. `useImageCache()` reads `isInitialized` via `useSyncExternalStore` and returns stable `getCachedImage`/`clearCache`/`getCacheStats` helpers — same shape as the old context value, so consumers (`AlbumArt`, `AlbumList`, `ArtistList`) are unchanged. No `<ImageCacheProvider>` in the `App.tsx` tree anymore.
...
- `persistentCache.ts` — (2026-09-10) two-tier stale-while-revalidate KV cache: synchronous in-memory `Map` front + IndexedDB (`xylonic-perf-cache` / `kv` store). `init()` opens IDB and hydrates the memory tier (memory-only fallback if IDB is unavailable); `get<T>` (sync, fresh only) / `peek<T>` (`{data, fresh, stale}`) / `set(key, data, ttlMs)` (memory now, IDB fire-and-forget) / `swr(key, fetcher, ttlMs)` (serve cached now, dedup + background refresh when stale, await only on hard miss) / `invalidate(prefix?)`. Bounded to `MAX_ENTRIES=400` (oldest evicted); entries past `ttlMs * STALE_FACTOR` (8×) are dead. `init()` is raced against a 200 ms timeout in `index.tsx` before first render. 9 unit tests (`persistentCache.test.ts`, memory-only path).
- `metadataCache.ts` — thin **facade** over `persistentCache` (2026-09-10; was a standalone in-memory Map). Same API — `get<T>(key)` / `set(key, data, ttlMs?)` (30-min default) / `invalidate(prefix?)` — but keys are `meta:`-prefixed and the data is now IDB-backed, so it survives reload / restart. Covers artists list, artist albums, album songs, paginated all-albums pages; `invalidate()` called in `AuthContext.login()` and `AuthContext.logout()`. New call sites wanting instant-stale + background refresh should use `persistentCache.swr(...)` directly.
