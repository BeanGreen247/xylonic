...
| 2026-09-10 | QueueTab virtualised (`react-window`, >60 rows) — from on-device CDP profiling (iPhone 15 Pro Max, iOS 18.7): a 3,330-song queue rendered **40,604 DOM nodes** (36k in `.right-panel-content`; 3,330 × 11-node rows), `content-visibility:auto` present but nodes/memory/reconcile cost unaffected. Row height measured a stable 85 px on device (was the blocker — "desktop vs touch height unknown") | every queue row in the DOM; only paint skipped | expect DOM for the panel to drop ~40k → a few hundred; **on-device re-measure pending** (user rebuilds, then CDP node count + scroll fps) | **provisional** — verify node count + that drag still works ≤60 and the add-menu still positions right |
